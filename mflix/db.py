...
def build_query_sort_project(filters):
    ...
    try:
        ...
        #query = {'$or': [{'title': filters}, {'fullplot': filters}, {'plot': filters}, {'genres': filters},{'cast': filters}]}
        query = {'title': filters}
        projection = {'title': 1}
    ...
    if filters:
        if "text" in filters:
            ...
            query = {'genres': {'$in': filters['genres']}} # EXPLICAR
...
def most_active_commenters():
    ...
    rc = ReadConcern(level=1) # you may want to change this read concern!
